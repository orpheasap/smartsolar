...
- Our 2nd html file, is almost identical to the first one. The only difference is that it subscribes to the mqtt topic `-t temp_chart`, and reads the messege, wich is a string with 12 readings with one space between them. The source code can be found [here](../master/code/smartsolar2.html). 
